...
import { checkInactiveThreads, cleanUpExpiredThreads } from '../plugins/tags/functions/InactiveThreads';
...
export default class ReadyListener extends Listener<'ready'> {
    ...
    public execute(): void {
        ...
        console.log('Starting Inactive Thread Checker.');        
        setInterval(async () => {
            try {
                await cleanUpExpiredThreads(this.ctx);
                await checkInactiveThreads(this.ctx);
            } catch (error) {
